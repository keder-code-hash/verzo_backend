...
exports.updateDryCleanerProfile = async (req, res) => {
    ...
    return res.status(200).json({ success: true, msg: "Dry cleaner profile updated successful"});
};
...
exports.myDryCleanerBooking = async (req, res) => {
    ...
    const dryCleanerDetails = await DryCleaning.findOne({ userId: req.body.dryCleanerId });
    if(!userDetails || !dryCleanerDetails) return res.status(200).json({ success: false, msg: "Invalid dryCleanerId"});

    let model = await dryCleanerBooking.create({ bookingBy: req.data.id, bookingTo: req.body.dryCleanerId });
...
exports.cancelDryCleaningOrder = async (req, res) => {
    ...
    if(!model) return res.status(200).json({ success: false, msg: "Invalid order id"});
