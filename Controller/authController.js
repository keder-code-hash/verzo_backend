...
exports.signup = async (req, res, next) => {
    try {
        ...
        if (check_user) return res.status(409).json({ success: false, error: "Phone number is already registered", userData: check_user._id});
        ...
        let new_user = new Auth({
            ...
            otp, qrCode:generateQR, accountType: "user"
        });
        ...
        // });
        return res.send({ success: true, msg: "Details saved", data: { user: save, token } });
    } catch (error) {
...
exports.login = async (req, res) => { 
    ...
    try {
        const { email, password, lat, lng, deviceId, deviceType, deviceToken, phoneNumber } = req.body;
        if(!email && !phoneNumber) return res .status(404).json({ error: "Email or Phone Number is required" });
        let user;
        ...
        if(!verifyPassword) return res.status(403).json({ error: "Invalid Password" });
        ...
        const payload = { id: user._id, name: `${user.firstName}`, email: user.email ? user.email : "" };
        let jwtoken = jwt.sign(payload, keys.secretOrKey, { expiresIn: 31556926 });
        return res.status(200).json({ success: true, msg: "Logged In", data: { token: jwtoken, user } });
    } catch (error) {
        return res.status(422).send({
            success: false,
            error: [{message: error.message}],
...
exports.phoneOtpVerification = async (req, res, next) => {
    try {
        ...
        if (isUser.otp == req.body.otp) {
            ...
            await isUser.save();
            var userdata = await Auth.findOne({ _id: isUser._id }).select("-password -devices -otp").exec();
            ...
            return res.status(404).json({ error: "OTP not matched..Please enter valid otp" });
        }
    } catch (error) {
        return res.status(422).send({
            success: false,
...
exports.emailOtpVerification = async (req, res, next) => {
    try {
        ...
        if (isUser.otp == req.body.otp) {
            ...
            await isUser.save();
            const userdata = await Auth.findOne({ _id: isUser._id }).select("-password -devices -otp").exec();
            const payload = { id: isUser._id, name: `${isUser.firstName}`, email: isUser.email ? isUser.email : "" };
            let jwtoken = jwt.sign(payload, keys.secretOrKey, { expiresIn: 31556926 });
            return res.status(200).json({
                success: true,
...
exports.profileSetup = async (req, res) => {
    ...
    try {
        const { firstName, lastName, email, country, state, pinCode, city } = req.body;
