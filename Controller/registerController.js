...
exports.signup = async (req, res) => {
    ...
    try {
        ...
        if (check_user) return res.json({ success: false, error: "Phone number is already used" });
        const hashedPassword = passwordHash.generate(password);
        let newUser = new Auth({ phoneNumber, password: hashedPassword, accountType: "user" });
        await newUser.save();
...
exports.login = async (req, res) => { 
    ...
    try {
        ...
        if(!user) return res.json({ success: false, error: "Phone number is is not registered yet" });
        const verifyPassword = passwordHash.verify( password, user.password );
        if(!verifyPassword) return res.json({ success: false, error: "Invalid password" });
...
exports.updateMyProfileImage = async (req, res) => {
    try {
        ...
        } else return res.status(422).send({ success: false, error: [{message: "File Not Found"}] });
